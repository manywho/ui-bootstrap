    ...
    class FileUpload extends React.Component<any, any> {
        ...
        onUpload = () => {
            if (this.state.fileNames.length > 0) {
                ...
                const model = !manywho.utils.isNullOrWhitespace(this.props.id) ? manywho.model.getComponent(this.props.id, this.props.flowKey) : false;
                ...
                return this.props.upload(this.props.flowKey, formData, (e: any) => {
                    if (e.lengthComputable)
                        this.setState({ progress: parseInt((e.loaded / e.total * 100).toString()) });
                })
                .done(response => {
                    this.setState({
                        isUploadDisabled: false,
                    ...
                    setTimeout(() => {
                        this.setState({ isUploadComplete: false, isProgressVisible: false, progress: 100 });
                    }, 2000);
        ...
        onFileSelected = (files) => {
            if (!this.props.isDesignTime) {
                ...
                const model = !manywho.utils.isNullOrWhitespace(this.props.id) && manywho.model.getComponent(this.props.id, this.props.flowKey);
        ...
        render() {
            ...
            if (model) {
                ...
                label = <label>{model.label}{model.isRequired ? <span className="input-required"> *</span> : null}</label>;
                validationMessage = <span className="help-block">{model.validationMessage || state.validationMessage}</span>;
            ...
            return <div className={componentClassName} id={this.props.id}>
                <div className="clearfix">
                    ...
                    <div className={'input-group ' + ((isAutoUpload) ? 'hidden' : '')}>
                        <input type="text" className={inputClassName} readOnly={true} value={this.state.fileNames.join(' ')} />
